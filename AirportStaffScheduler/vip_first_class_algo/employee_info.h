...
private:
    ...
    int32_t qualification_mask_;                              ///< 资质掩码（位掩码）
    int64_t total_work_time_;                                 ///< 累计工作时长（秒）
    map<string, int32_t> shift_type_counts_;       ///< 班次类型次数统计（键为"主班1"、"主班2"、"副班1"等）
    map<string, int32_t> hall_task_counts_;        ///< 厅房任务次数统计（键为厅房名称）
    vector<int64_t> assigned_task_ids_;                 ///< 分配的任务ID列表
};
